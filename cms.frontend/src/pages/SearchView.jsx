...
const SearchView = ({ navigate }) => {
  ...
  const results = query.trim() === '' ? [] : productsData.filter(p =>
    ...
    <div className="page-container page-transition">
      ...
      <div className="search-bar-large">
        <input
          ...
          placeholder="Nhập tên giày, áo, quần, vớ..."
          value={query}
      ...
      {query && results.length > 0 && (
        <div className="products-grid">
          {results.map(product => <ProductCard key={product.id} product={product} navigate={navigate} />)}
        </div>
      )}
      {query && results.length === 0 && (
        <div className="empty-state">Không tìm thấy sản phẩm nào phù hợp với "{query}".</div>
      )}
    </div>
  );
};
