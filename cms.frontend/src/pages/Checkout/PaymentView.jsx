...
const PaymentView = ({ navigate, clearCart, cart }) => {
  ...
  const handlePayment = async () => {
    ...
    // Bật Loading Overlay (người dùng sẽ thấy màn hình làm mờ cực ngầu)
    setLoading(true);
    ...
    try {
      // BƯỚC 1: Thực hiện gọi API lưu đơn hàng vào CSDL (rất nhanh)
      const response = await orderService.checkout(orderPayload);

      if (response && response.orderId) {

        // BƯỚC 2: Gọi API gửi Email xác nhận CHẠY NGẦM (Fire and Forget)
        // Không dùng await để không bắt người dùng phải chờ 3 giây!
        orderService.sendOrderEmail(response.orderId).catch(err => {
          console.error("Lỗi gửi email ngầm:", err);
        });

        // Chỉ delay thêm 500ms để hiệu ứng Loading trông đẹp mắt trước khi tắt
        setTimeout(() => {
          setLoading(false);
          ...
          // Chuyển hướng sang trang Sản phẩm để kích thích khách hàng tiếp tục mua sắm
          navigate('products');
        }, 500);
